...
impl DecoderLinear {
    ...
    pub fn from_varstore(
        ...
        let linear1 = nn::Linear::new(
            varstore.get((64, in_features), names[0])?,
            Some(varstore.get(64, names_bias[0])?),
        );
        let prelu = nn::PReLU::new(varstore.get(1, names[1])?, true);
        let linear2 = nn::Linear::new(
            varstore.get((out_features, 64), names[2])?,
            Some(varstore.get(out_features, names_bias[1])?),
        );
